...
def sound_read(file_name):
    ...
    return sound
sound_hammer = sound_read("hammer")
sound_list = [sound_read("hit1"), sound_read("hit2"), sound_read("hit3"),
              sound_read("hit4"), sound_read("hit5")]
# 두더지 class
class mole:
            ...
            for mole in mole_list:
                if mole.clicked == False:
                    ...
                    if x >= x1 and x <= x1+w and y >= y1 and y <= y1+h:
                        ...
                        if game_over == False:
                            game_point += 1
                            sound_index = random.randrange(0,5)
